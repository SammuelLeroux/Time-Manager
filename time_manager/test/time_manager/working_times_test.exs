defmodule TimeManager.WorkingTimesTest do
  ...
  describe "working_times" do
    ...
    test "create_working_time/1 with valid data creates a working_time" do
      valid_attrs = %{start: ~N[2024-10-07 11:57:00], end: ~N[2024-10-07 11:57:00]}

      assert {:ok, %WorkingTime{} = working_time} = WorkingTimes.create_working_time(valid_attrs)
      assert working_time.start == ~N[2024-10-07 11:57:00]
      assert working_time.end == ~N[2024-10-07 11:57:00]
    end
    ...
    test "update_working_time/2 with valid data updates the working_time" do
      working_time = working_time_fixture()
      update_attrs = %{start: ~N[2024-10-08 11:57:00], end: ~N[2024-10-08 11:57:00]}

      assert {:ok, %WorkingTime{} = working_time} = WorkingTimes.update_working_time(working_time, update_attrs)
      assert working_time.start == ~N[2024-10-08 11:57:00]
      assert working_time.end == ~N[2024-10-08 11:57:00]
    end
